...
class _TableDisplayState extends State<TableDisplay>
  ...
  Future<void>? shopOrderListFuture;
    ...
    ObjectBoxStore.initStore().then((_) {
      checkAndInsertInitialData().then((_) {
        fetchData().then((_) {
          ...
          // shopOrderListFuture = getShopOrderList();
          // workOrderListFuture!.then((_) {
          //   shopOrderListFuture = getShopOrderList();
          // });
        });
      });
  ...
  Future<List<dynamic>> fetchCombinedData() async{
    return Future.wait([
      workOrderListFuture!,  // make sure it's initialized before calling
      shopOrderListFuture!,
    ]);
  }

  Future<void> combineData() async{
    await workOrderListFuture;
    await shopOrderListFuture;
  }
  ...
  Future<void> getWorkOrderList() async {
    ...
    String filterString = "(Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'RELEASED') or (Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'STARTED') or (Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'WORKREQUEST') or (Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'PREPARED') or (Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'UNDERPREPARATION') or (Objstate eq IfsApp.ActiveWorkOrdersHandling.ActiveSeparateState'WORKDONE')";
    ...
    String apiEndPointShop =
        "main/ifsapplications/projection/v1/ShopOrdersHandling.svc/ShopOrds"; //api endpoint

    // Generate the filter string dynamically based on the workstation IDs
    String filterStringShop = "(Contract eq 'IHI') or (Contract eq 'I2S01')";

    Map<String, dynamic>? queryParametersShop = {
      // "\$filter":"(WorkTypeId eq 'I2S-100') or (WorkTypeId eq 'I2S-110') or (WorkTypeId eq 'I2S-120') or (WorkTypeId eq 'I2S-130')",
      // "\$filter":filterString,
      "\$orderby": "OrderNo"
    };
      ...
      queryParametersShop["\$filter"] = filterStringShop;

      //call the getWithParameters method to get the work tasks
      var responseShop = await serverCall.getWithParameters(UserData.accessToken,
          apiEndPointShop, queryParametersShop, widget.refreshTokenFunction);

      List<Map<String, dynamic>>shopOrdersOnline = [];
      if (responseShop.statusCode == 200) {
        var data = json.decode(responseShop.body);
        for (var i = 0; i < data["value"].length; i++) {
          shopOrdersOnline.add({
            'OrderNo': data["value"][i]["OrderNo"],
            'OrderCode': data["value"][i]["OrderCode"],
            'PartNo': data["value"][i]["PartNo"],
            'Objstate': data["value"][i]["Objstate"],
            'OrgDueDate': data["value"][i]["OrgDueDate"],
            'Contract': data["value"][i]["Contract"]
          }); //add the record to the workTasks array as a map
          print("wo no : ${shopOrdersOnline[i]}");
          print("count : $i");
        }
      } else if (responseShop.body == 'Token refresh failed') {
        if (context.mounted) {
          showLogoutPopup(context, widget.logOutFunction);
        }
      } else {
        if (context.mounted) {
          HttpErrorHandler.showStatusDialog(
              context, responseShop.statusCode, responseShop.reasonPhrase!);
        }
      }
  ...
  Future<void> getShopOrderList() async {
    print("from online get work order list");
    final serverCall = await apiService.Methods();
    String apiEndPoint =
        "main/ifsapplications/projection/v1/ShopOrdersHandling.svc/ShopOrds"; //api endpoint
    ...
    // Generate the filter string dynamically based on the workstation IDs
    String filterString = "(Contract eq 'IHI') or (Contract eq 'I2S01')";
  ...
  Future<void> checkAndInsertInitialData() async {
    ...
    if (isFirstRun) {
      ...
      List<WorkStation> initialWorkStations = [
        WorkStation(id: 1, workStationId: 'I2S-100', left: 0.95, top: 0.53,category: 'workOrder',filter: 'RELEASED'),
        WorkStation(id: 2, workStationId: 'I2S-110', left: 0.53, top: 0.35, category: 'workOrder',filter: 'STARTED'),
        WorkStation(id: 3, workStationId: 'I2S-120', left: 0.17, top: 0.59, category: 'shopOrder',filter: 'IHI'),
        WorkStation(id: 4, workStationId: 'I2S-130', left: 0.16, top: 0.39, category: 'shopOrder',filter: 'I2S01'),
        WorkStation(id: 5, workStationId: 'I2S-140', left: 0.76, top: 0.34, category: 'workOrder',filter: 'WORKREQUEST'),
        WorkStation(id: 6, workStationId: 'I2S-150', left: 0.80, top: 0.68, category: 'workOrder',filter: 'PREPARED'),
        WorkStation(id: 7, workStationId: 'I2S-160', left: 0.43, top: 0.53, category: 'workOrder',filter: 'UNDERPREPARATION'),
        WorkStation(id: 8, workStationId: 'I2S-170', left: 0.77, top: 0.53, category: 'workOrder',filter: 'WORKDONE'),
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Text(
                                  selectedCategory == 'shopOrder' ? "Shop Orders" : "Work Orders",
                            ...
                            Expanded(
                              child: SingleChildScrollView(
                                child: DataTable(
                                  columns: selectedCategory == 'shopOrder'
                                      ? const [
                                    DataColumn(label: Text("OrderNo")),
                                    DataColumn(label: Text("Objstate")),
                                    DataColumn(label: Text("OrgDueDate")),
                                  ]
                                      : const [
                                    DataColumn(label: Text("WoNo")),
                                    DataColumn(label: Text("WorkTypeId")),
                                    DataColumn(label: Text("Latest Finish")),
                                  ],
                                  rows: selectedWorkOrders.map((order) {
                                    DateTime? date;
                                    String dateField =
                                    selectedCategory == 'shopOrder' ? 'OrgDueDate' : 'LatestFinish';

                                    try {
                                      date = DateTime.parse(order[dateField]);
                                    } catch (_) {
                                      date = null;
                                    }
                                    ...
                                    return DataRow(cells: [
                                      DataCell(Text(selectedCategory == 'shopOrder'
                                          ? order["OrderNo"].toString()
                                          : order["WoNo"].toString())),
                                      DataCell(Text(selectedCategory == 'shopOrder'
                                          ? order["Objstate"].toString()
                                          : order["WorkTypeId"].toString())),
                                      DataCell(
                                        Container(
  ...
  void handleTap(Offset tapPosition) {
    for (int i = 0; i < rects.length; i++) {
      if (rects[i].contains(tapPosition)) {
        final workstation = storedWorkStations[i];
        String? filter = workstation.filter;
        String? category = workstation.category;

        print("Clicked WorkStation ID: ${workstation.workStationId}, Category: $category");
        ...
        setState(() {
          ...
          if (category == 'workOrder') {
            selectedWorkOrders = workOrders
                .where((wo) =>
            (wo["Objstate"]?.toString().toLowerCase() ?? '') ==
                filter?.toLowerCase())
                .toList();
          } else if (category == 'shopOrder') {
            selectedWorkOrders = shopOrders
                .where((so) =>
            (so["Contract"]?.toString().toLowerCase() ?? '') ==
                filter?.toLowerCase())
                .toList();
          } else {
            selectedWorkOrders = [];
...
class RectanglePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (int i = 0; i < rects.length; i++) {
      ...
      WorkStation ws = storedWorkStations[i];
      int totalWorkCount = 0;

      if (ws.category == 'workOrder') {
        totalWorkCount = workOrders
            .where((wo) => (wo['Objstate']?.toString().toLowerCase() ?? '') == ws.filter.toLowerCase())
            .length;
      }else if(ws.category == 'shopOrder'){
        totalWorkCount = shopOrders
            .where((wo) => (wo['Contract']?.toString().toLowerCase() ?? '') == ws.filter.toLowerCase())
            .length;
      }
